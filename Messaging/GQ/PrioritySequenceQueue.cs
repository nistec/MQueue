...
namespace Nistec.Messaging
{
    public interface IPrioritySequenceQueue
    {
        ...
        */
        ...
        IQueueMessage RemoveItem(string ptr);

        bool ItemExists(string ptr);

        #endregion
    }

    public abstract class PrioritySequenceQueue : /*ITransScop,*/ IDisposable, IPrioritySequenceQueue
    {
        //internal static PrioritySequenceQueue Factory(IQProperties prop)
        //{
        //    switch (prop.Mode)
        //    {
        ...
        private SequenceQueue normalQ;
        private SequenceQueue mediumQ;
        private SequenceQueue highQ;

        SyncTimerDispatcher<IQueueMessage> transactionalItems;

       //ConcurrentDictionary<Guid,IQueueMessage> TransactionalItems;
        ...
        protected virtual void OnErrorOccured(QueueItemEventArgs e)
        {
            if (ErrorOccured != null)
                ErrorOccured(this, e);
            Logger.Error("PrioritySequenceQueue OnError ", e.Message);
        }
        */
        protected virtual void OnTryAdd(string ptr, IQueueMessage item, bool result)
        {
            if (item != null)
                Logger.Debug("OnTryAdd {0} item:{1}", result, item.Print());
        }
        protected virtual void OnTryDequeue(string ptr, IQueueMessage item, bool result)
        {
            if (item != null)
                Logger.Debug("TryDequeue {0} item:{1}", result, item.Print());
        }
        protected virtual void OnTryPeek(string ptr, IQueueMessage item, bool result)
        {
            if (item != null)
        ...
        protected abstract bool TryAdd(string identifier, IQueueMessage item);

        protected abstract bool TryDequeue(string identifier, out IQueueMessage item);

        protected abstract bool TryPeek(string identifier, out IQueueMessage item);
        ...
        */
        public abstract bool ItemExists(string ptr);

        //protected abstract bool TransBegin(Guid ptr, out IQueueMessage item);

        #endregion

        #region ctor

        public PrioritySequenceQueue(string name,int consumeInterval=100)
        {
            ConsumeInterval = Math.Max(consumeInterval,10);
            this.Name = name;
            this.isTrans = false;
            normalQ = new SequenceQueue();
            mediumQ = new SequenceQueue();
            highQ = new SequenceQueue();
        ...
        //internal PrioritySequenceQueue(string host)
        //{
        //    this.host = host;
        ...
        */
        ...
        {
            private PrioritySequenceQueue owner;
            ...
            public TransScop(PrioritySequenceQueue owner,IQueueMessage item)
            {
                start = item.ArrivedTime;
                this.owner=owner;
                this.Item = item;
                this.Guid = item.ItemId;
            ...
            {
                Console.WriteLine("Trans scope:{0}", Guid);
                ...
                try
                {
                    ...
                    using (TransactionScope scope = new TransactionScope())
                    {
                        ...
                        Console.WriteLine("Complete the transaction scope:{0}", Guid);
        ...
        */
        ...
        private delegate void RemoveItemCallback(string ptr);


        private void RemoveAsyncWorker(string ptr)
        {
            try
            {
                IQueueMessage item;
        ...
        /// </summary>
        /// <returns></returns>
        private void RemoveAsync(string ptr)
        {
        ...
        public virtual void Clear()
        {
            ...
            ClearItems();
            Logger.Info("PrioritySequenceQueue Cleared");
        }
        ...
        /// <returns></returns>
        private IQueueMessage DequeueScop(Guid ptr, bool transactional = false)
        {
        ...
        */
        ...
        /// </summary>
        /// <returns></returns>
        public virtual IQueueMessage Dequeue(string ptr)
        {
            IQueueMessage item = null;
            try
            {
                if (TryDequeue(ptr, out item))
                {
                    ...
                    if (item != null)
                        Logger.Info("PrioritySequenceQueue Dequeue Guid ", item.Print());
                    return item;
                }
            ...
            {
                Logger.Exception("PrioritySequenceQueue Dequeue Guid error ", tex);
            }
            catch (Exception ex)
            {
                Logger.Exception("PrioritySequenceQueue Dequeue Guid error ", ex);
            }
        ...
        public virtual IQueueMessage Dequeue(Priority priority)
        {
            ...
            try
            {
                ...
                DequeueScopEvent(item);
                if (item != null)
                    Logger.Info("PrioritySequenceQueue Dequeue Priority ", item.Print());
            }
            catch (Exception ex)
            {
                Logger.Exception("PrioritySequenceQueue Dequeue error ", ex);
            }
        ...
        public virtual bool TryDequeue(out IQueueMessage item)
        {
            ...
            try
            {
                ...
                if (item != null)
                    Logger.Info("PrioritySequenceQueue TryDequeue ", item.Print());
        ...
        public virtual IQueueMessage Dequeue()
        {
            ...
            try
            {
                ...
                DequeueScopEvent(item);
                if (item != null)
                    Logger.Info("PrioritySequenceQueue Dequeue ", item.Print());
            }
            catch (Exception ex)
            {
                Logger.Exception("PrioritySequenceQueue Dequeue error ", ex);
        ...
        public virtual IQueueMessage Consume(int maxSecondWait)
        {
            ...
            try
            {
                ...
                DequeueScopEvent(item);
                if (item != null)
                    Logger.Info("PrioritySequenceQueue Consume ", item.Print());
            }
            catch (Exception ex)
            {
                Logger.Exception("PrioritySequenceQueue Consume error ", ex);
            }
        ...
        */
        ...
        public virtual IQueueAck Enqueue(IQueueMessage item)
        {
            ...
            //{
              ...
              ((QueueMessage)item).SetArrived();
            //Guid ptr = new Guid(item, Host);
             string ptr = item.Identifier;
            ...
            if (TryAdd(ptr, item))
            {
                ...
                */
                if (item != null)
                    Logger.Info("PrioritySequenceQueue Enqueue ", item.Print());
                return new QueueAck(MessageState.Arrived, item);// new Guid(ptr, PtrState.Arrived);
            }
            if (item != null)
                Logger.Error("PrioritySequenceQueue FailedEnqueue ", item.Print());
            //}
        ...
        public virtual IQueueAck Requeue(IQueueMessage item)
        {
            ...
            if (!ItemExists(ptr))
            {
                ...
                {
                    if (item != null)
                        Logger.Error("PrioritySequenceQueue Requeue FailedEnqueue ", item.Print());
                    return new QueueAck(MessageState.FailedEnqueue, item);
                }
            }
            if (item != null)
                Logger.Info("PrioritySequenceQueue Requeue ", item.Print());
        ...
        /// </summary>
        /// <param name="ptr"></param>
        public virtual IQueueMessage RemoveItem(string ptr)
        {
            IQueueMessage item;
        ...
        public virtual IQueueAck HoldAdd(IQueueMessage item)
        {
            ...
            if (TryAdd(ptr, item))
            {
              ...
              //  holdQ.Enqueue(ptr);
                ...
                return new QueueAck(MessageState.Holded, item);// new Guid(ptr, PtrState.Arrived);
            }
