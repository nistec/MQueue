...
{
    public interface IPriorityGQueue
    {
    ...
    public abstract class PriorityGQueue : /*ITransScop,*/ IDisposable, IPriorityQueue
    {
        //internal static PriorityGQueue Factory(IQProperties prop)
        //{
        //    switch (prop.Mode)
        //    {
        ...
        private GPtrQueue normalQ;
        private GPtrQueue mediumQ;
        private GPtrQueue highQ;
       ...
       //ConcurrentDictionary<Ptr,IQueueMessage> TransactionalItems;
        ...
        protected virtual void OnErrorOccured(QueueItemEventArgs e)
        {
            if (ErrorOccured != null)
                ErrorOccured(this, e);
            Logger.Error("PriorityGQueue OnError ", e.Message);
        }
        */
        protected virtual void OnTryAdd(string identifier, IQueueMessage item, bool result)
        {
            if (item != null)
                Logger.Debug("OnTryAdd {0} item:{1}", result, item.Print());
        }
        protected virtual void OnTryDequeue(string identifier, IQueueMessage item, bool result)
        {
            if (item != null)
                Logger.Debug("TryDequeue {0} item:{1}", result, item.Print());
        }
        protected virtual void OnTryPeek(string identifier, IQueueMessage item, bool result)
        {
            if (item != null)
        ...
        */
        ...
        public PriorityGQueue(string name,int consumeInterval=100)
        {
            ConsumeInterval = Math.Max(consumeInterval,10);
            this.Name = name;
            this.isTrans = false;
            normalQ = new GPtrQueue();
            mediumQ = new GPtrQueue();
            highQ = new GPtrQueue();
        ...
        //internal PriorityGQueue(string host)
        //{
        //    this.host = host;
        ...
        */
        ...
        private class TransScop
        {
            ...
            public TransScop(PriorityGQueue owner,IQueueMessage item)
            {
        ...
        */
        ...
        private void RemoveAsyncWorker(Ptr ptr)
        {
            ...
            {
                IQueueMessage item;
                TryDequeue(ptr.Identifier, out item);
            }
            catch (Exception ex)
            {
        ...
        public virtual void Clear()
        {
            ...
            ClearItems();
            Logger.Info("PriorityGQueue Cleared");
        }
        ...
        public IQueueMessage Peek(Ptr ptr)
        {
            ...
            TryPeek(ptr.Identifier,out item);
            return item;
        }
        ...
        */
        ...
        /// </summary>
        /// <returns></returns>
        public virtual IQueueMessage Dequeue(string Identifier)
        {
            IQueueMessage item = null;
            try
            {
                if (TryDequeue(Identifier, out item))
                {
                    DequeueScopEvent(item);
                    if (item != null)
                        Logger.Info("PriorityGQueue Dequeue Ptr ", item.Print());
                    return item;
                }
            ...
            {
                Logger.Exception("PriorityGQueue Dequeue Ptr error ", tex);
            }
            catch (Exception ex)
            {
                Logger.Exception("PriorityGQueue Dequeue Ptr error ", ex);
            }
        ...
        public virtual IQueueMessage Dequeue(Ptr ptr)
        {
            ...
            try
            {
                if (TryDequeue(ptr.Identifier, out item))
                {
                    DequeueScopEvent(item);
                    if (item != null)
                        Logger.Info("PriorityGQueue Dequeue Ptr ", item.Print());
        ...
        public virtual IQueueMessage Dequeue(Priority priority)
        {
            ...
            try
            {
                    ...
                    switch (priority)
                    {
                        case Priority.Normal:
                            if (normalQ.TryDequeue(out ptr))
                                TryDequeue(ptr.Identifier, out item);// item = DequeueScop(ptr);
                            break;
                        case Priority.Medium:
                ...
                DequeueScopEvent(item);
                if (item != null)
                    Logger.Info("PriorityGQueue Dequeue Priority ", item.Print());
            }
            catch (Exception ex)
            {
                Logger.Exception("PriorityGQueue Dequeue error ", ex);
            }
        ...
        public virtual bool TryDequeue(out IQueueMessage item)
        {
            ...
            try
            {
                ...
                if (item != null)
                    Logger.Info("PriorityGQueue TryDequeue ", item.Print());
        ...
        public virtual IQueueMessage Dequeue()
        {
            ...
            try
            {
                ...
                DequeueScopEvent(item);
                if (item != null)
                    Logger.Info("PriorityGQueue Dequeue ", item.Print());
            }
            catch (Exception ex)
            {
                Logger.Exception("PriorityGQueue Dequeue error ", ex);
        ...
        public virtual IQueueMessage Consume(int maxSecondWait)
        {
            ...
            try
            {
                ...
                DequeueScopEvent(item);
                if (item != null)
                    Logger.Info("PriorityGQueue Consume ", item.Print());
            }
            catch (Exception ex)
            {
                Logger.Exception("PriorityGQueue Consume error ", ex);
            }
        ...
        */
        ...
        public virtual IQueueAck Enqueue(IQueueMessage item)
        {
            ...
            if (TryAdd(ptr.Identifier, item))
            {
                ...
                */
                if (item != null)
                    Logger.Info("PriorityGQueue Enqueue ", item.Print());
                return new QueueAck(MessageState.Arrived, item);// new Ptr(ptr, PtrState.Arrived);
            }
            if (item != null)
                Logger.Error("PriorityGQueue FailedEnqueue ", item.Print());
            //}
        ...
        public virtual IQueueAck Requeue(IQueueMessage item)
        {
            ...
            if (!ItemExists(ptr))
            {
                ...
                {
                    if (item != null)
                        Logger.Error("PriorityGQueue Requeue FailedEnqueue ", item.Print());
                    return new QueueAck(MessageState.FailedEnqueue, item);
                }
            }
            if (item != null)
                Logger.Info("PriorityGQueue Requeue ", item.Print());
