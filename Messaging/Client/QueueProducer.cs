...
namespace Nistec.Messaging.Client
{
    public class QueueProducer: QueueClient
    {
        ...
        public IQueueAck PublishItem(QueueApi q, QueueItem item, int connectTimeOut)
        {
            return _Api.PublishItem(item, connectTimeOut);
        }

        public void PublishItem(QueueApi q, QueueItem item, int connectTimeOut, Action<IQueueAck> action)
        {
            _Api.PublishItem(item, connectTimeOut, action);
        }


        public void EnqueueAsync(QueueApi api, QueueItem item, int connectTimeOut, Action<IQueueAck> action)
        {
            _Api.EnqueueAsync(item, connectTimeOut, action);
        }
        public IQueueAck Enqueue(QueueApi api, string queueName, QueueItem item, int connectTimeOut)
        {
            return _Api.Enqueue(item, connectTimeOut);
        }

        public static void EnqueueAsync(string hostAddress, string queueName, QueueItem item, int connectTimeOut, Action<IQueueAck> action)
        {
            var api = GetApi(queueName, hostAddress);

            api.EnqueueAsync(item, connectTimeOut, action);
        }
        public static IQueueAck Enqueue(string hostAddress, string queueName, QueueItem item, int connectTimeOut)
        {
            var api = GetApi(queueName, hostAddress);
