...
namespace Nistec.Messaging.DbSLite
{
    ...
    public class DbLiteMessageContext : EntityContext<DbLiteMessage>
    {
        public DbLiteMessageContext() : base() { }

        /// <summary>
        /// Get all the services as list of entities <see cref="MessageEntity"/> from database.
        /// </summary>
        /// <returns></returns>
        public static IList<DbLiteMessage> GetItems()
        {
        ...
        /// <returns></returns>
        public void WriteStream(DbLiteMessage entity, NetStream stream)
        {
            this.Set(entity);
            this.EntityWrite(stream, null);
        }

        public int Enqueue(DbLiteMessage message)
        {
            this.Set(message);
        ...
        {
            return this.ExecuteCommand<byte[]>("sp_peek_fifo_item", DataParameter.Get("Host", host, "Identifier", identifier), CommandType.StoredProcedure);
        }

        public byte[] PeekStream(string host)
        {
            return this.ExecuteCommand<byte[]>("sp_peek_fifo", DataParameter.Get("Host", host), CommandType.StoredProcedure);
        }

        public byte[] DequeueStream(string host)
        {
            return this.ExecuteCommand<byte[]>("sp_dequeue_fifo", DataParameter.Get("Host", host), CommandType.StoredProcedure);
        }

        public byte[] DequeueStreamTrans(string host, int transTimeout)
        {
            return this.ExecuteCommand<byte[]>("sp_dequeue_fifo_trans", DataParameter.Get("Host", host, "Expiration", transTimeout), CommandType.StoredProcedure);
        }

        public byte[] DequeueStream(string host, string identifier)
        {
            return this.ExecuteCommand<byte[]>("sp_dequeue_fifo_item", DataParameter.Get("Host", host, "Identifier", identifier), CommandType.StoredProcedure);
        }

        public byte[] DequeueStreamTrans(string host, string identifier, int transTimeout)
        {
            return this.ExecuteCommand<byte[]>("sp_dequeue_fifo_trans_item", DataParameter.Get("Host", host, "Identifier", identifier, "Expiration", transTimeout), CommandType.StoredProcedure);
        }
        ...
        {
            this.ExecuteNonQuery("sp_fifo_commit", DataParameter.Get("Identifier", identifier), CommandType.StoredProcedure);
        }

        public void TransAbort(string identifier, int maxRetry)
        {
            this.ExecuteNonQuery("sp_fifo_abort", DataParameter.Get("Identifier", identifier, "MaxRetry", maxRetry), CommandType.StoredProcedure);
        }
        ...
        {
            return this.ExecuteCommand<int>("sp_fifo_count", DataParameter.Get("Host", host), CommandType.StoredProcedure);
        }

        public int ClearAllItems(string host)
        {
            return this.ExecuteNonQuery("sp_fifo_clear", DataParameter.Get("Host", host), CommandType.StoredProcedure);
        }
        ...
        {
            byte[] bytes = null;
            using (DbLiteMessageContext context = new DbLiteMessageContext())
            {
                if (isTrans)
