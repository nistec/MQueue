...
namespace Nistec.Messaging
{
    ...
    public sealed class PriorityComplexGQueue : PriorityGQueue
    {
        ...
        public PriorityComplexGQueue(IQProperties qp)
            : base(qp.QueueName, qp.ConsumeInterval)
        {
            ...
            if (CoverMode == CoverMode.Persistent)
            {
                ...
                if (qp.ReloadOnStart)
                    Logger.Info("PriorityComplexGQueue will load items to : {0}", qp.QueueName);
                else
                    Logger.Info("PriorityComplexGQueue will clear all items from : {0}", qp.QueueName);
        ...
        {
            Logger.Info("PriorityComplexGQueue ClearCompleted : {0}", m_db.Name);
        }
        ...
        {
            QLogger.Error("PriorityComplexGQueue ErrorOcurred : {0}", e.Args);
        }

        private void M_db_LoadCompleted(object sender, Generic.GenericEventArgs<string, int> e)
        {
            QLogger.Info("PriorityComplexGQueue LoadCompleted : {0}, Count:{1}", e.Args1, e.Args2);
        }

        private void M_db_BeginLoading(object sender, EventArgs e)
        {
            QLogger.Info("PriorityComplexGQueue BeginLoading : {0}", m_db.Name);
        }
        ...
        bool PersistItemRemove(string Identifier)
        {
            ...
            Task.Run(() =>
            {
                m_db.TryRemove(Identifier, out persistItem);
            });
            return true;
        ...
        bool PersistItemAdd(string Identifier, IQueueMessage item)
        {
            //return m_db.TryAdd(ptr.Identifier, item);

            Task.Run(() =>
            {
                if(m_db.TryAdd(Identifier, item))
                {
                    OnTryAdd(Identifier, item, true);
                }
            });
            return true;
        ...
        protected override bool TryAdd(string Identifier, IQueueMessage item)
        {
            var copy = item.Copy();
            QueueItems[Identifier] = copy;

            if (CoverMode == CoverMode.Persistent)
            {
                if (CommitMode == CommitMode.OnDisk)
                {
                    if (m_db.TryAdd(Identifier, copy))
                    {
                        OnTryAdd(Identifier, item, true);
                        return true;
                    }
                ...
                {
                    return PersistItemAdd(Identifier, copy);
                }
                return false;
            }
            else //if (CommitMode == CommitMode.OnMemory)
        ...
        protected override bool TryPeek(string Identifier, out IQueueMessage item)
        {
            if (CoverMode == CoverMode.Persistent)
            {
                ...
                if (CommitMode == CommitMode.OnDisk)
                {
                    if (m_db.TryGetValue(Identifier, out item))
                    {
                        OnTryPeek(Identifier, item, true);
                        return true;
                    }
                }
                else if (QueueItems.TryGetValue(Identifier, out item))
                {
                    return true;
                }
            }
        ...
        protected override bool TryDequeue(string Identifier, out IQueueMessage item)
        {

            if (CoverMode == CoverMode.Persistent)
            {
                if (QueueItems.TryFetch(Identifier, out item))
                {
                    if (CommitMode == CommitMode.OnDisk)
                    {
                        IQueueMessage item_pers = null;
                        if (m_db.TryRemove(Identifier, out item_pers))
                        {
                            OnTryDequeue(Identifier, item, true);
                            return true;
                        }
                    }
                    else
                    {
                        return PersistItemRemove(Identifier);
                    }
                }
            }
            else
        ...
        */
        ...
        public override bool ItemExists(Ptr ptr)
        {
            try
            {
                if (QueueItems.ContainsKey(ptr.Identifier))
                {
            ...
            catch (Exception ex)
            {
                ...
                Logger.Error("PriorityComplexGQueue ItemExists : Host:{0}, message:{1}", this.Name, ex.Message);
            }
            return false;
        }
