...
namespace MControl.QueueServer.Monitoring
{
	...
	/// </summary>
	public class MonitoringServerSession : TcpServerTask
	{
        ...
        private void BeginReadCmd()
        {
            ...
            try{
                SmartStream.ReadLineAsyncOperation readLineOp = new SmartStream.ReadLineAsyncOperation(new byte[32000],SizeExceededAction.JunkAndThrowException);
                // This event is raised only when read next coomand completes asynchronously.
                readLineOp.Completed += new EventHandler<EventArgs<SmartStream.ReadLineAsyncOperation>>(delegate(object sender,EventArgs<SmartStream.ReadLineAsyncOperation> e){                
        ...
        private bool ProcessCmd(SmartStream.ReadLineAsyncOperation op)
        {
			// Check errors.
            if(op.Error != null){
                OnError(op.Error);
            ...
            try{
                ...
                foreach(DataRow dr in dsSettings.Tables["Users"].Rows){
                    if(dr["UserName"].ToString() == TextUtils.UnQuoteString(args[0]) && dr["Password"].ToString() == TextUtils.UnQuoteString(args[1])){
                        ...
                        m_User = new GenericIdentity(dr["UserName"].ToString());
        ...
        private void GetServerInfo()
        {
            ...
            */
            ...
            try{
                ...
                ds.Tables["ServerInfo"].Columns.Add("MqTasks");
                ds.Tables["ServerInfo"].Columns.Add("Pop3Sessions");
                ds.Tables["ServerInfo"].Columns.Add("Imap_Sessions");
                ...
                foreach(object session in allSessions){
                    try{
                        if(session is MqTask){
                            startReads  += ((MqTask)session).TcpStream.BytesReaded;
                            startWrites += ((MqTask)session).TcpStream.BytesWritten;
                            smtpSessions++;
                        }
                ...
                foreach(object session in allSessions){
                    try{
                        if(session is MqTask){
                            endReads  += ((MqTask)session).TcpStream.BytesReaded;
                            endWrites += ((MqTask)session).TcpStream.BytesWritten;
                        }
                        else if(session is Pop3_Session){
                ...
                dr["MailServerVersion"] =ConfigInfo.MailServerVersion;
                ...
                dr["MqTasks"]      = smtpSessions;
                dr["Pop3Sessions"]      = pop3Sessions;
                dr["Imap_Sessions"]      = imapSessions;
        ...
        private void GetSessions()
        {
            ...
            */
            ...
            //  dt.Columns.Add("VirtualServer");
                ...
                foreach(object session in allSessions){
                    if(session is MqTask){
                        transferRatesHolder.Add(session,new object[]{((MqTask)session).TcpStream.BytesReaded,((MqTask)session).TcpStream.BytesWritten});
                    }
                    else if(session is Pop3_Session){
                ...
                foreach(object session in allSessions){
                    try{
                        if(session is MqTask){
                            MqTask smtpSession = (MqTask)session;
        ...
        //                        c.Parse(new MControl.Messaging.Net.StringReader(contact));
        //                        contacts.Add(c);
        //                    }
        //                }
        ...
        private void GetLogSessions(string argsText)
        {
            ...
            */
            ...
            try{
                ...
                string fileName = "";
                if(args[1] == "Smtp"){
                    fileName = virtualServer.SmtpLogsPath + "smtp";
                }
                else if(args[1] == "Pop3"){
                    fileName = virtualServer.Pop3_LogsPath + "pop3";
                }
                else if(args[1] == "Imap"){
                    fileName = virtualServer.ImapLogsPath + "imap";
                }
                    ...
                    if(virtualServer.ID.ToLower() == args[0].ToLower()){ 
                        ...
                        foreach(Imap_MessageInfo message in messages){
                            try{
                                ...
                                dr["Envelope"] = Imap_Fetch_r_i_Envelope.ConstructEnvelope(Mail_Message.ParseFromByte(msgItems.Header));
                ...
                foreach(VirtualServer virtualServer in this.Server.MailServer.VirtualServers){
                    if(virtualServer.ID.ToLower() == args[0].ToLower()){
                        ...
                        try{
                            ...
                            if(t.GetInterface("MControl.Messaging.Net.MailServer.ISmtpMessageFilter") != null){
                                type = "ISmtpMessageFilter";
                            }
                            else if(t.GetInterface("MControl.Messaging.Net.MailServer.ISmtpSenderFilter") != null){
                                type = "ISmtpSenderFilter";
                            }
                            else if(t.GetInterface("MControl.Messaging.Net.MailServer.ISmtpUserMessageFilter") != null){
                        ...
                        foreach(string file in files){
                            try{                                
                                ...
                                using(FileStream fs = new FileStream(file,FileMode.Open,FileAccess.Read,FileShare.Read)){                
                                    Mime_Header_Collection h = new Mime_Header_Collection(new Mime_Header_Provider());
