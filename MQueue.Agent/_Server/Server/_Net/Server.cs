...
using MControl.QueueServer.Monitoring;
...
namespace MControl.QueueServer
{
    ...
    public class Server : IDisposable
    {
        ...
        private Timer               m_SettingsTimer    = null;
        private DateTime            m_ServersFileDate   = DateTime.MinValue;
        private MonitoringServer    m_ManagementServer = null;
        private DateTime            m_StartTime;
        private List<VirtualServer> m_VirtualServers   = null;
        ...
        public Server()
        {
            m_VirtualServers = new List<VirtualServer>();
            ...
            m_SettingsTimer = new Timer(10000);
            m_SettingsTimer.AutoReset = true;
            m_SettingsTimer.Elapsed += new ElapsedEventHandler(m_SettingsTimer_Elapsed);
        }
        ...
        #region method m_SettingsTimer_Elapsed

        private void m_SettingsTimer_Elapsed(object sender, ElapsedEventArgs e)
        {
            LoadVirtualServers();
        }
        ...
        public void Start()
        {
            ...
            m_SettingsTimer_Elapsed(this,null);
            m_SettingsTimer.Enabled = true;

            // Start management server
            m_ManagementServer = new MonitoringServer(this);
		    m_ManagementServer.Start();
        ...
        {
            get{ return m_VirtualServers.ToArray(); }
        }
        ...
        public object[] Sessions
        {
            get{
                ...
                foreach(VirtualServer virtualServer in m_VirtualServers){
                    if(virtualServer.Enabled){
                        if(virtualServer.Smtp_Server.IsRunning){
                            sessions.AddRange(virtualServer.Smtp_Server.Sessions.ToArray());
                        }
                        if(virtualServer.Pop3Server.IsRunning){
                            sessions.AddRange(virtualServer.Pop3Server.Sessions.ToArray());
                        }
                        if(virtualServer.Imap_Server.IsRunning){
                            sessions.AddRange(virtualServer.Imap_Server.Sessions.ToArray());
                        }
                ...
                sessions.AddRange(m_ManagementServer.Sessions.ToArray());
